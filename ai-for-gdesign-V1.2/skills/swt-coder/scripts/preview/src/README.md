...
└── assets/
    ...
    └── themes/          # SWT 主题体系（换肤）
        ├── base.css       # 字体/骨架/滚动条
        ├── swt-bridge.css # --swt-* → --el-* 桥接（Element Plus 跟随换肤）
        └── swt-default.css # 默认皮肤（协议见同目录 README.md）
```
...
4. 在工程路由中注册页面，例如：
   ...
   ```
5. 在工程入口引入主题三件套（见 `main.js`）：`base.css`、`swt-bridge.css`、`themes/swt-default.css`。
6. 页面颜色全部走 `var(--swt-*)` token —— 换肤体系接入后页面自动跟随（协议见同目录 README.md）。

## 换肤

- 运行时切换：`document.documentElement.setAttribute('data-swt-theme', '<皮肤名>')`
- 新增皮肤：`assets/themes/swt-{name}.css` + 入口追加 import（或预览 html 追加 link）
