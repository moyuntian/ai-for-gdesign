...
// {dir}/preview-data.js — the source map consumed by index.swt.html to
// compile SFCs in the browser via vue3-sfc-loader.
//
// <style lang="less"> blocks in .vue files are compiled browser-side by less.js
// (loaded in index.swt.html). Imported .less files are handled by the
...
export function refresh(dir) {
  ...
  if (!existsSync(join(root, 'index.swt.html'))) {
    return { ok: false, reason: `index.swt.html not found: ${join(root, 'index.swt.html')}` };
  }
  ...
  const body =
    ...
    `// key = path from src root; consumed by index.swt.html + vue3-sfc-loader\n` +
    `window.__SWT_SRC__ = ${JSON.stringify(map, null, 0)};\n` +
    `window.__SWT_REFRESHED_AT = ${JSON.stringify(stamp)};\n`;
...
if (process.argv[1] && resolve(process.argv[1]) === resolve(fileURLToPath(import.meta.url))) {
  ...
  if (!dir) {
    console.log('RESULT: FAIL | Usage: node build-data.mjs --dir "<folder with src/ and index.swt.html>"');
    process.exit(1);
  }
